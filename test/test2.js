  ...
  describe("PayToClickContract", function () {
    ...
    describe("Pay to click test suite", async function () {
      ...
      before(async () => {
        ...
        //token manager
        const TokenManagerFactory = await hre.ethers.getContractFactory("TokenManagerContract");
      ...
      it("should be able to buy membership", async () => {
        ...
        // await PayToClickContract.connect(memberSeven).buyMembershipPlan(
        //   2,
        //   memberOne.address
        // );
  
        // await PayToClickContract.connect(memberEight).buyMembershipPlan(
        //   2,
        //   memberOne.address
        // );
  
        // await PayToClickContract.connect(memberNine).buyMembershipPlan(
        //   0,
        //   memberOne.address
        // );
      ...
      //   await PayToClickContract.connect(memberOne).clickToEarn({
      ...
      it("should be able to withdraw earnings", async () => {
        const balBefore = await VUSDContract.balanceOf(memberOne.address);
        const contractbalBefore = await VUSDContract.balanceOf(
          TokenManager.address
        );
        console.log(
          "contract balance before : ",
          +contractbalBefore.toString() / 10 ** 18
        );
        await PayToClickContract.connect(memberOne).withdrawEarnings({
          value: ethers.utils.parseEther("1"),
        });
        const balAfter = await VUSDContract.balanceOf(memberOne.address);
        const contractbalAfter = await VUSDContract.balanceOf(
          TokenManager.address
        );
  
        console.log(
          "user bal before: user balance after : ",
          +balBefore.toString() / 10 ** 18,
          +balAfter.toString() / 10 ** 18
        );
        console.log(
          "contract balance after : ",
          +contractbalAfter.toString() / 10 ** 18
        );
        expect(+balAfter.toString() / 10 ** 18).to.be.greaterThan(
          +balBefore.toString() / 10 ** 18
        );
        expect(+contractbalBefore.toString() / 10 ** 18).to.be.greaterThan(
          +contractbalAfter.toString() / 10 ** 18
        );
      });
  
      it("should check total earnings", async () => {
        const [, , , , directRefEarnings, , totalEarnings, ,] =
          await PayToClickContract.connect(memberOne).getMemberDetails();
          console.log("total earnings ", +totalEarnings.toString()/10 ** 18)
          console.log("direct referrals ",  +directRefEarnings.toString()/10** 18)
        expect(+totalEarnings.toString() / 10 ** 18).to.be.equal(300);
      });
      it("should buy another membership", async () => {
        // await PayToClickContract.connect(memberEleven).buyMembershipPlan(
        //   2,
        //   memberOne.address
        // );
        const balBeforeSubing = await VUSDContract.balanceOf(
          memberOne.address
        );
        console.log("balance before subbing ", (+balBeforeSubing.toString())/ 10 ** 18)
        await PayToClickContract.connect(memberOne).renewSubScription({
          value: ethers.utils.parseEther("1"),
        });
        // await PayToClickContract.connect(memberTen).buyMembershipPlan(
        //   2,
        //   memberOne.address
        // );
        
        const balAfterSubing = await VUSDContract.balanceOf(
          memberOne.address
        );
        console.log("balance after subbing ", (+balAfterSubing.toString())/ 10 ** 18)
        const balBefore = await VUSDContract.balanceOf(memberOne.address);
        const contractbalBefore = await VUSDContract.balanceOf(
          TokenManager.address
        );
        console.log(
          "contract subscrit balance || before : ",
          +contractbalBefore.toString() / 10 ** 18
        );
        await PayToClickContract.connect(memberOne).withdrawEarnings({
          value: ethers.utils.parseEther("1"),
